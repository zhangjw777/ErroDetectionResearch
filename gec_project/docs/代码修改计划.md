...
## 评估与预测模块优化记录 (2024-12-04)

### 优化背景
原有的 `evaluate.py` 和 `predictor.py` 存在以下问题：
1. 数据格式解析错误：`type='positive'` 实际表示无错（正样本），被错误理解
2. 句级评估不完整：缺少 F0.5 指标
3. 输出方式不友好：直接在控制台打印，不便于后续分析
4. predictor 不支持字典格式的批量数据输入
...
| 修改类型 | 说明 |
|---------|------|
| 新增方法 | `predict_from_data()` - 支持字典格式数据批量预测 |
| 新增方法 | `save_predictions_to_csv()` - 预测结果保存到CSV |
| 修改方法 | `predict_batch()` - 添加 return_svo 参数 |
| 新增函数 | `_run_demo_mode()` - 演示模式 |
| 修改 main | 支持命令行参数 --return_svo, --input_path, --output_path |

#### evaluate.py 修改
| 修改类型 | 说明 |
|---------|------|
| 新增方法 | `_compute_f_score()` - 统一计算 F-beta 分数 |
| 新增方法 | `save_results_to_csv()` - 评估结果保存到CSV |
| 修改方法 | `_normalize_data()` - 修正 type 字段解析 (positive=无错, negative=有错) |
| 修改方法 | `evaluate_sentence_level()` - 添加 F0.5 指标，添加详细记录收集 |
| 修改方法 | `evaluate_char_level()` - 添加详细记录收集 |
| 修改方法 | `evaluate()` - 添加 collect_details 参数 |
| 修改方法 | `print_report()` - 改为日志输出，减少控制台刷屏 |
| 修改 main | 添加 --output_path, --no_details 参数，结果保存到文件 |
...
```
- `positive`: 正样本，句子无错 (source == target 的语义)
- `negative`: 负样本，句子有错 (source != target 的语义)

### 使用示例
```bash
# 批量预测（数据在代码中通过datasets库加载）
python predictor.py --exp_name exp_xxx

# 带SVO信息的预测
python predictor.py --exp_name exp_xxx --return_svo

# 评估
python evaluate.py --exp_name exp_xxx --data_path eval_data.json --output_path eval_results

# 不保存详细记录的评估
python evaluate.py --exp_name exp_xxx --data_path eval_data.json --no_details
```

### 输出文件说明
- `eval_results.csv`: 评估指标汇总
- `eval_results.json`: 评估指标 JSON 格式
- `eval_results_sent_details.csv`: 句级评估详细记录
- `predictions.csv`: 预测结果详细记录
...
### 1. 混淆集外部化
**修改文件**: `src/config.py`, `data/vocab/confusions.json`

**变更说明**:
- 将 `POLITICAL_CONFUSIONS` 字典从 `config.py` 移动到单独的 JSON 文件 `data/vocab/confusions.json`
- 在 `config.py` 中添加 `_load_confusions()` 函数动态加载混淆集
- 新增 `CONFUSIONS_FILE` 配置项指向混淆集文件路径
- `augmentation.py` 无需修改，继续从 `config` 导入 `POLITICAL_CONFUSIONS`
...
### 2. 配置保存到实验目录
**修改文件**: `src/config.py`, `src/trainer.py`

**变更说明**:
- 在 `Config` 类中新增 `save_to_experiment(exp_dir, filename="config.yaml")` 方法
- 在 `trainer.py` 的 `Trainer.__init__` 中，创建实验目录后自动调用保存配置
- 保存格式为 YAML，便于人类阅读
- 配置文件保存在 `experiments/exp_xxx/config.yaml`（与 tensorboard 目录平级）

### 3. predictor.py 简化
**修改文件**: `src/predictor.py`

**变更说明**:
- 移除命令行参数: `--input_path`, `--output_path`, `--device`
- 保留命令行参数: `--model_path`, `--exp_name`, `--return_svo`
- 移除 `_run_demo_mode()` 演示函数
- 默认使用 CUDA 设备（如可用）
- 数据输入改为代码内部通过 datasets 库加载，预留 `data` 变量供用户填充

**新的使用方式**:
```python
# 在 predictor.py 中直接修改数据加载代码
# 或导入 GEDPredictor 类在其他脚本中使用

from predictor import GEDPredictor
predictor = GEDPredictor(model_path, vocab_dir, device)

# 数据格式: {'source': [...], 'target': [...], 'type': [...]}
data = {
    'source': [...],
    'target': [...],
    'type': [...]
}
predictions = predictor.predict_from_data(data, return_svo=True)
predictor.save_predictions_to_csv(predictions, output_path, include_svo=True)
```
