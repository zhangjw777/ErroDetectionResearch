...
class GEDDataset(Dataset):
    ...
    def __init__(
        ...
        Args:
            ...
            ged_label_map: GED标签到ID的映射
            svo_label_map: SVO标签到ID的映射
            max_length: 最大序列长度
        """
        self.data_path = data_path
        self.tokenizer = tokenizer
        self.ged_label_map = ged_label_map
    ...
    def __getitem__(self, idx):
        ...
        for i, token in enumerate(tokens):
            ...
            ged_aligned_labels.append(ged_label_ids[i])
            ...
            for sub_token_id in token_ids[1:]:
                ...
                ged_aligned_labels.append(-100)  # ignore
                svo_aligned_labels.append(-100)
                label_mask.append(0)
        
        # 添加[SEP]
        input_ids.append(self.tokenizer.sep_token_id)
        ged_aligned_labels.append(-100)
        svo_aligned_labels.append(-100)
        label_mask.append(0)
        ...
        if seq_length > self.max_length:
            ...
            ged_aligned_labels = ged_aligned_labels[:self.max_length]
            ...
            ged_aligned_labels += [-100] * padding_length
        ...
        return {
            ...
            'ged_labels': torch.tensor(ged_aligned_labels, dtype=torch.long),
...
def build_label_maps(vocab_dir: str) -> Tuple[Dict, Dict]:
    ...
    Returns:
        ged_label_map: GED标签 -> ID
        svo_label_map: SVO标签 -> ID
    """
    # 加载GED标签
    ged_label_path = f"{vocab_dir}/label_map.txt"
    ...
    keep_id = ged_label_map.get(cfg.GEC_KEEP_LABEL)
    if keep_id != 0:
        raise ValueError(
    ...
    logger.info(f"Loaded {len(ged_label_map)} GED labels and {len(svo_label_map)} SVO labels")
...
if __name__ == "__main__":
    ...
    ged_label_map, svo_label_map = build_label_maps(cfg.vocab_dir)
